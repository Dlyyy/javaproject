...
public class RangeVariableResolver
{
  ...
  RangeVariableResolver(Session paramSession, QuerySpecification paramQuerySpecification)
  {
  ...
  RangeVariableResolver(Session paramSession, RangeVariable[] paramArrayOfRangeVariable, Expression paramExpression, ParserDQL.CompileContext paramCompileContext, boolean paramBoolean)
  {
    this.session = paramSession;
  ...
  void processConditions()
  {
    ...
    for (i = 0; i < this.queryConditions.size(); i++)
    {
      ...
      if ((!((Expression)localObject).isTrue()) && ((((Expression)localObject).isSingleColumnEqual) || (((Expression)localObject).isColumnCondition)))
      {
  ...
  static Expression decomposeAndConditions(Session paramSession, Expression paramExpression, HsqlList paramHsqlList)
  {
    ...
    if (i == 49)
    {
      ...
      if (localExpression1.isTrue()) {
        return localExpression2;
      }
      if (localExpression2.isTrue()) {
    ...
    if ((i == 40) && (localExpression1.getType() == 25) && (localExpression2.getType() == 25))
    {
  ...
  static Expression decomposeOrConditions(Expression paramExpression, HsqlList paramHsqlList)
  {
    ...
    if (i == 50)
    {
      ...
      if (localExpression1.isFalse()) {
        return localExpression2;
      }
      if (localExpression2.isFalse()) {
        return localExpression1;
      }
  ...
  void expandConditions()
  {
    ...
    for (i = 0; i < this.firstOuterJoinIndex; i++)
    {
      ...
      for (int n = 0; n < localHsqlArrayList.size(); n++)
      {
        ...
        if (!((Expression)localObject1).isTrue()) {
  ...
  void reorderRanges(HsqlArrayList paramHsqlArrayList1, HsqlArrayList paramHsqlArrayList2)
  {
    ...
    for (int j = 0; j < this.firstLeftJoinIndex; j++)
    {
      ...
      if (!(localTable instanceof TableDerived))
      {
        ...
        Index.IndexUse[] arrayOfIndexUse1 = localTable.getIndexForColumns(this.session, this.colIndexSetEqual, 40, false);
        localObject2 = null;
        for (int n = 0; n < arrayOfIndexUse1.length; n++)
        {
          localObject2 = arrayOfIndexUse1[n].index;
          double d2 = searchCost(this.session, localTable, (Index)localObject2, arrayOfIndexUse1[n].columnCount, 40);
          if (d2 < d1)
          {
            d1 = d2;
            i = j;
        ...
        if (localObject2 == null)
        {
          ...
          while (localIterator.hasNext())
          {
            ...
            if (localObject2 != null)
            {
              d1 = localTable.getRowStore(this.session).elementCount() / 2.0D;
    ...
    for (i = 1; i < this.firstLeftJoinIndex; i++)
    {
      ...
      if (k == 0)
      {
        for (m = 0; m < paramHsqlArrayList1.size(); m++)
        {
          ...
          Index.IndexUse[] arrayOfIndexUse2 = ((Table)localObject2).getIndexForColumns(this.session, this.colIndexSetEqual, 40, false);
  ...
  int getJoinedRangePosition(Expression paramExpression, int paramInt, RangeVariable[] paramArrayOfRangeVariable)
  {
    int i = -1;
    this.tempSet.clear();
    paramExpression.getJoinRangeVariables(paramArrayOfRangeVariable, this.tempSet);
    for (int j = 0; j < this.tempSet.size(); j++) {
      for (int k = 0; k < paramArrayOfRangeVariable.length; k++) {
        if ((this.tempSet.get(j) == paramArrayOfRangeVariable[k]) && (k >= paramInt))
        {
          if (i > 0) {
            return -1;
          }
    ...
    while (i < j)
    {
      ...
      if ((localExpression1 != null) && (localExpression1.isIndexable(paramRangeVariableConditions.rangeVar)))
      {
        ...
        case 2: 
          break;
        case 40: 
          if ((localExpression1.getSubType() != 52) && (localExpression1.getSubType() != 51) && (localExpression1.getLeftNode().getRangeVariable() == paramRangeVariableConditions.rangeVar))
          {
    ...
    if ((!bool) && (paramBoolean))
    {
      ...
      while (j < k)
      {
        ...
        if (localExpression2 != null) {
          ...
          else if ((localExpression2.getType() == 40) && (localExpression2.exprSubType == 52) && (paramInt < this.firstLeftJoinIndex) && (this.firstRightJoinIndex == this.rangeVariables.length) && (!localExpression2.getRightNode().isCorrelated()))
          {
            ...
            Index.IndexUse[] arrayOfIndexUse = paramRangeVariableConditions.rangeVar.rangeTable.getIndexForColumns(this.session, localOrderedIntHashSet, 40, false);
  ...
  private void setEqualityConditions(RangeVariable.RangeVariableConditions paramRangeVariableConditions, HsqlList paramHsqlList, int paramInt)
  {
    ...
    if (localIndex == null)
    {
      localObject1 = paramRangeVariableConditions.rangeVar.rangeTable.getIndexForColumns(this.session, this.colIndexSetEqual, 40, false);
      ...
      if (localObject1.length > 1) {
        for (j = 0; j < localObject1.length; j++)
        {
          ...
          double d2 = ((PersistentStore)localObject2).searchCost(this.session, localObject1[j].index, localObject1[j].columnCount, 40);
          if (d2 < d1)
          {
            d1 = d2;
    ...
    for (int j = 0; j < paramHsqlList.size(); j++)
    {
      ...
      if (localObject2 != null)
      {
        ...
        if (((m == 40) || (m == 47)) && (((Expression)localObject2).getLeftNode().getRangeVariable() == paramRangeVariableConditions.rangeVar) && (((Expression)localObject2).isIndexable(paramRangeVariableConditions.rangeVar)))
        {
  ...
  void setInConditionsAsTables()
  {
    for (int i = this.rangeVariables.length - 1; i >= 0; i--)
    {
      ...
      if (localExpressionLogical1 != null)
      {
        ...
        Index.IndexUse[] arrayOfIndexUse = localRangeVariable1.rangeTable.getIndexForColumns(this.session, localOrderedIntHashSet, 40, false);
  ...
  private double searchCost(Session paramSession, Table paramTable, Index paramIndex, int paramInt1, int paramInt2)
  {
    if ((paramTable instanceof TableDerived)) {
      return 1000.0D;
    }
    return paramTable.getRowStore(paramSession).searchCost(paramSession, paramIndex, paramInt1, paramInt2);
  }
}


/* Location:              E:\java\java学习\hutubill\lib\all.jar!\org\hsqldb\RangeVariableResolver.class
