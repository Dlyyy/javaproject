...
public class GranteeManager
{
  ...
  public void grant(Session paramSession, OrderedHashSet paramOrderedHashSet, SchemaObject paramSchemaObject, Right paramRight, Grantee paramGrantee, boolean paramBoolean)
  {
    if ((paramSchemaObject instanceof RoutineSchema))
    {
      localObject = ((RoutineSchema)paramSchemaObject).getSpecificRoutines();
      grant(paramSession, paramOrderedHashSet, (SchemaObject[])localObject, paramRight, paramGrantee, paramBoolean);
      return;
    }
    ...
    if (!paramGrantee.isAccessible(paramSchemaObject)) {
      throw Error.error(2000, paramGrantee.getName().getNameString());
    }
    if (!paramGrantee.isGrantable(paramSchemaObject, paramRight))
    {
      paramSession.addWarning(Error.error(1007, paramGrantee.getName().getNameString()));
      return;
    }
  ...
  public void grant(Session paramSession, OrderedHashSet paramOrderedHashSet, SchemaObject[] paramArrayOfSchemaObject, Right paramRight, Grantee paramGrantee, boolean paramBoolean)
  {
    int i = 0;
    for (int j = 0; j < paramArrayOfSchemaObject.length; j++) {
      if (paramGrantee.isGrantable(paramArrayOfSchemaObject[j], paramRight))
      {
        grant(paramSession, paramOrderedHashSet, paramArrayOfSchemaObject[j], paramRight, paramGrantee, paramBoolean);
        i = 1;
      }
    }
    if (i == 0) {
  ...
  public void updateAddColumn(HsqlNameManager.HsqlName paramHsqlName1, HsqlNameManager.HsqlName paramHsqlName2)
  {
    Iterator localIterator = getRoles().iterator();
    Grantee localGrantee;
    while (localIterator.hasNext())
    {
      localGrantee = (Grantee)localIterator.next();
      localGrantee.updateRightsForNewColumn(paramHsqlName1, paramHsqlName2);
    }
    ...
    updateAddColumn(paramHsqlName1);
  }
  
  private void updateAddColumn(HsqlNameManager.HsqlName paramHsqlName)
  {
    Iterator localIterator = getRoles().iterator();
    Grantee localGrantee;
    while (localIterator.hasNext())
    {
      localGrantee = (Grantee)localIterator.next();
      localGrantee.updateRightsForNewColumn(paramHsqlName);
    }
    localIterator = getGrantees().iterator();
    while (localIterator.hasNext())
    {
  ...
  public void setDigestAlgo(String paramString)
  {
    this.digestAlgo = paramString;
  }
  ...
  synchronized MessageDigest getDigester()
  {
    if (this.digester == null) {
      ...
      {
        throw Error.error(458, localNoSuchAlgorithmException);
      }
    }
    ...
    throws RuntimeException
  {
    ...
    {
      throw Error.error(458, localUnsupportedEncodingException);
    }
    byte[] arrayOfByte = getDigester().digest(arrayOfByte);
...
/* Location:              E:\java\java学习\hutubill\lib\all.jar!\org\hsqldb\rights\GranteeManager.class
