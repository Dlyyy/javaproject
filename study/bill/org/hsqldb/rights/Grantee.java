  ...
  implements SchemaObject
{
  ...
  void updateRightsForNewColumn(HsqlNameManager.HsqlName paramHsqlName1, HsqlNameManager.HsqlName paramHsqlName2)
  {
    Iterator localIterator = this.directRightsMap.get(paramHsqlName1);
    for (Right localRight = null; localIterator.hasNext(); localRight = (Right)localIterator.next()) {}
    if (localRight == null) {
      return;
    }
    localRight.addNewColumn(paramHsqlName2);
    updateAllRights();
  }
  
  void updateRightsForNewColumn(HsqlNameManager.HsqlName paramHsqlName)
  {
    Iterator localIterator = this.grantedRightsMap.get(paramHsqlName);
    for (Right localRight = null; localIterator.hasNext(); localRight = (Right)localIterator.next()) {}
    if (localRight == null) {
      return;
    }
  ...
  public OrderedHashSet getAllDirectPrivileges(SchemaObject paramSchemaObject)
  {
    if (paramSchemaObject.getOwner() == this)
    {
      ...
      return (OrderedHashSet)localObject;
    }
  ...
  public boolean hasNonSelectTableRight(SchemaObject paramSchemaObject)
  {
    ...
    return localRight.canAccesssNonSelect();
  }
  ...
  HsqlArrayList getRightsSQL()
  {
    ...
    while (localIterator1.hasNext())
    {
      ...
      while (localIterator2.hasNext())
      {
        ...
        case 3: 
        case 4: 
          Table localTable = this.granteeManager.database.schemaManager.findUserTable(localHsqlName.name, localHsqlName.schema.name);
          if (localTable != null)
          {
...
/* Location:              E:\java\java学习\hutubill\lib\all.jar!\org\hsqldb\rights\Grantee.class
