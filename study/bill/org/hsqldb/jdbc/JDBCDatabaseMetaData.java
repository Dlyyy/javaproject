...
{
  static final Integer INT_COLUMNS_NO_NULLS = Integer.valueOf(0);
  ...
  private void setCurrentProperties()
    throws SQLException
  {
    ResultSet localResultSet = executeSelect("SYSTEM_PROPERTIES", "PROPERTY_NAME IN ('sql.concat_nulls', 'sql.nulls_first' , 'sql.nulls_order')");
    while (localResultSet.next())
    {
      String str = localResultSet.getString(2);
      boolean bool = Boolean.valueOf(localResultSet.getString(3)).booleanValue();
      if (str.equals("sql.concat_nulls")) {
        this.concatNulls = bool;
      } else if (str.equals("sql.nulls_first")) {
        this.nullsFirst = bool;
      } else if (str.equals("sql.nulls_order")) {
    ...
    throws SQLException
  {
    ...
    String str = localResultSet.next() ? localResultSet.getString(1) : null;
    localResultSet.close();
    return str;
  }
  ...
  {
    ResultSet localResultSet = executeSelect("SYSTEM_SCHEMAS", "IS_DEFAULT=TRUE");
    String str = localResultSet.next() ? localResultSet.getString(2) : null;
    localResultSet.close();
    return str;
  }
...
/* Location:              E:\java\java学习\hutubill\lib\all.jar!\org\hsqldb\jdbc\JDBCDatabaseMetaData.class
