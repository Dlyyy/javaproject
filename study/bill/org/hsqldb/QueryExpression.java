  ...
  implements RangeGroup
{
  ...
  public void resolve(Session paramSession, RangeGroup[] paramArrayOfRangeGroup, Type[] paramArrayOfType)
  {
    ...
    if (paramArrayOfType != null) {
      for (int i = 0; (i < this.unionColumnTypes.length) && (i < paramArrayOfType.length); i++) {
  ...
  void resolveTypesPartTwo(Session paramSession)
  {
    ...
    if (this.isRecursive)
    {
      this.leftQueryExpression.resolveTypesPartTwoRecursive(paramSession);
      this.recursiveTable.colTypes = this.leftQueryExpression.getColumnTypes();
      for (int i = 0; i < this.recursiveTable.colTypes.length; i++) {
        this.recursiveTable.getColumn(i).setType(this.recursiveTable.colTypes[i]);
      }
      this.recursiveTable.getFullIndex(paramSession);
    }
    else
    {
    ...
    for (int j = 0; j < this.leftQueryExpression.unionColumnMap.length; j++)
    {
      int k = this.leftQueryExpression.unionColumnMap[j];
      int m = this.rightQueryExpression.unionColumnMap[j];
      ColumnBase localColumnBase = localResultMetaData1.columns[k];
      int n = localResultMetaData1.columns[k].getNullability();
      byte b = localResultMetaData2.columns[m].getNullability();
      if ((b == 1) || ((b == 2) && (n == 0)))
      {
        if ((localColumnBase instanceof ColumnSchema))
        {
          localColumnBase = new ColumnBase();
          localColumnBase.setType(this.leftQueryExpression.unionColumnTypes[j]);
          localResultMetaData1.columns[k] = localColumnBase;
        }
        localColumnBase.setNullability(b);
      }
    }
  ...
  void resolveTypesPartThree(Session paramSession)
  {
    ...
    if (this.isRecursive) {
      this.recursiveTable.queryExpression.isCorrelated = false;
    }
  }
  ...
  Result getResultRecursive(Session paramSession)
  {
    ...
    RowSetNavigatorData localRowSetNavigatorData1 = this.recursiveTable.getNavigator(paramSession);
    ...
    for (int i = 0;; i++)
    {
      ...
      int j = localRowSetNavigatorData1.getSize();
      switch (this.unionType)
      {
      case 1: 
      ...
      if (j == localRowSetNavigatorData1.getSize()) {
        break;
      }
      if (i > 256) {
  ...
  {
    TableDerived localTableDerived = (TableDerived)getResultTable();
    return localTableDerived.columnList;
  }
...
/* Location:              E:\java\java学习\hutubill\lib\all.jar!\org\hsqldb\QueryExpression.class
