  ...
  implements SchemaObject
{
  ...
  public String getSQL()
  {
    ...
    for (int i = 0; i < this.columnCount; i++)
    {
      ...
      if ((!((Type)localObject1).isDistinctType()) && (!((Type)localObject1).isDomainType()) && (((Type)localObject1).isCharacterType()))
      {
        localObject2 = ((CharacterType)localObject1).getCollation();
        if (((Collation)localObject2).isObjectCollation()) {
          localStringBuffer.append(' ').append(((Collation)localObject2).getCollateSQL());
        }
      }
      ...
      if (localColumnSchema.isAutoUpdate())
      {
        localStringBuffer.append(' ').append("ON").append(' ');
        localStringBuffer.append("UPDATE").append(' ');
        localStringBuffer.append(localColumnSchema.getUpdateExpression().getSQL());
      }
  ...
  public String getSQLForTableSpace()
  {
    ...
    localStringBuffer.append("SET").append(' ').append("TABLE").append(' ');
    localStringBuffer.append(getName().getSchemaQualifiedStatementName());
    localStringBuffer.append(' ').append("SPACE").append(' ').append(this.tableSpace);
    return localStringBuffer.toString();
  }
  ...
  public String[] getTriggerSQL()
  {
    ...
    for (int i = 0; i < this.triggerList.length; i++) {
      if (!this.triggerList[i].isSystem()) {
        localHsqlArrayList.add(this.triggerList[i].getSQL());
      }
    }
    String[] arrayOfString = new String[localHsqlArrayList.size()];
    localHsqlArrayList.toArray(arrayOfString);
  ...
  public String getIndexRootsSQL(long[] paramArrayOfLong)
  {
    ...
    localStringBuffer.append(' ');
    localStringBuffer.append(StringUtil.getList(new long[this.indexList.length], " ", ""));
    localStringBuffer.append(' ').append(this.store.elementCount());
    localStringBuffer.append('\'');
    return localStringBuffer.toString();
  }
  ...
  void updateConstraintLists()
  {
    ...
    for (m = 0; m < this.constraintList.length; m++) {
      ...
      case 1: 
        ...
        if (this.constraintList[m].hasCoreTriggeredAction())
        {
          this.referentialActions += 1;
          if (this.constraintList[m].getDeleteAction() == 0) {
            this.cascadingDeletes += 1;
          }
        }
        j++;
        break;
  ...
  void verifyConstraintsIntegrity()
  {
    for (int i = 0; i < this.constraintList.length; i++)
    {
      ...
      {
        if (localConstraint.getMain() != this.database.schemaManager.findUserTable(localConstraint.getMain().getName().name, localConstraint.getMain().getName().schema.name)) {
          throw Error.runtimeError(201, "FK mismatch : " + localConstraint.getName().name);
        }
        if (localConstraint.getRef() != this.database.schemaManager.findUserTable(localConstraint.getRef().getName().name, localConstraint.getRef().getName().schema.name)) {
          throw Error.runtimeError(201, "FK mismatch : " + localConstraint.getName().name);
        }
      }
    }
  ...
  {
    return hasPrimaryKey() ? this.constraintList[0] : null;
  }
    ...
    for (int i = 0; i < this.fkMainConstraints.length; i++)
    {
      ...
      if (localTable != this) {
        if (paramArrayOfInt == null)
        {
          ...
          {
            int[] arrayOfInt2 = localConstraint.getDeleteAction() == 0 ? null : localConstraint.getRefColumns();
  ...
  public boolean hasUpdatedColumn(int[] paramArrayOfInt)
  {
    return (this.hasUpdatedValues) && (!ArrayUtil.isAnyIntIndexInBooleanArray(paramArrayOfInt, this.colUpdated));
  }
  ...
  void checkReferentialColumnsMatch(int[] paramArrayOfInt1, Table paramTable, int[] paramArrayOfInt2)
  {
    for (int i = 0; i < paramArrayOfInt1.length; i++)
    {
      ...
      if (!localType1.canCompareDirect(localType2)) {
  ...
  {
    int i = 0;
    if ((paramConstraint != null) && (paramConstraint.getConstraintType() == 4)) {
      i = 1;
    }
    Object localObject1;
    if (this.isText)
    ...
    ((Table)localObject1).tableSpace = this.tableSpace;
    for (int j = 0; j < this.columnCount; j++)
    {
      ...
      {
        localColumnSchema = localColumnSchema.duplicate();
    ...
    for (int k = 1; k < this.indexList.length; k++)
    {
      ...
      if (!paramOrderedHashSet2.contains(localIndex1.getName()))
      {
        ...
        ((Table)localObject1).addIndexStructure(localIndex2);
      }
    }
    if (paramIndex != null)
    {
      paramIndex.setTable((TableBase)localObject1);
      ((Table)localObject1).addIndexStructure(paramIndex);
    }
    HsqlArrayList localHsqlArrayList = new HsqlArrayList();
    if (i != 0)
    {
  ...
  void checkColumnInCheckConstraint(int paramInt)
  {
    ...
    {
      Constraint localConstraint = this.constraintList[i];
      if ((localConstraint.getConstraintType() == 3) && (!localConstraint.isNotNull()) && (localConstraint.hasColumn(paramInt)))
      {
  ...
  public OrderedHashSet getUniquePKConstraintNames()
  {
    ...
    {
      Constraint localConstraint = this.constraintList[i];
      if ((localConstraint.getConstraintType() == 2) || (localConstraint.getConstraintType() == 4)) {
  ...
  int[] getUniqueNotNullColumnGroup(boolean[] paramArrayOfBoolean)
  {
    ...
    while (i < j)
    {
      ...
      else if (localConstraint.getConstraintType() == 4)
      {
        arrayOfInt = localConstraint.getMainColumns();
  ...
  public void createPrimaryKey(HsqlNameManager.HsqlName paramHsqlName, int[] paramArrayOfInt, boolean paramBoolean)
  {
    ...
    ArrayUtil.projectRow(this.colTypes, paramArrayOfInt, arrayOfType);
    ...
    createPrimaryIndex(paramArrayOfInt, arrayOfType, localHsqlName);
  ...
  {
    setSingleColumnTypeVars(paramInt);
    resetDefaultsFlag();
  }
  
  private void setSingleColumnTypeVars(int paramInt)
  {
    ...
    this.colUpdated[paramInt] = localColumnSchema.isAutoUpdate();
  }
  ...
  public Object[] getNewRowData(Session paramSession)
  {
  ...
  Index createIndexForColumns(Session paramSession, int[] paramArrayOfInt)
  {
    ...
    try
    {
      localIndex = createAndAddIndexStructure(paramSession, localHsqlName, paramArrayOfInt, null, null, false, false, false);
    }
  ...
  public void enforceRowConstraints(Session paramSession, Object[] paramArrayOfObject)
  {
    for (int i = 0; i < this.columnCount; i++)
    {
      ...
      if ((this.colNotNull[i] != 0) && (paramArrayOfObject[i] == null))
      {
        ...
        if ((localConstraint == null) && (ArrayUtil.find(getPrimaryKey(), i) > -1)) {
  ...
  public void enforceTypeLimits(Session paramSession, Object[] paramArrayOfObject)
  {
    ...
    {
      int j = localHsqlException.getErrorCode();
      if ((j == 62135) || (j == 62133) || (j == 62128))
      {
        ColumnSchema localColumnSchema = getColumn(i);
        String[] arrayOfString = { "", this.tableName.statementName, localColumnSchema.getName().statementName };
        throw Error.error(localHsqlException, j, 3, arrayOfString);
      }
  ...
  {
    for (int i = 0; i < this.indexList.length; i++) {
      if (ArrayUtil.haveEqualSets(this.indexList[i].getColumns(), paramArrayOfInt, paramArrayOfInt.length)) {
        return this.indexList[i];
      }
    }
    return null;
  ...
  synchronized Index.IndexUse[] getIndexForColumns(Session paramSession, OrderedIntHashSet paramOrderedIntHashSet, int paramInt, boolean paramBoolean)
  {
    ...
    Index.IndexUse[] arrayOfIndexUse = findIndexForColumns(paramSession, paramOrderedIntHashSet, paramInt, paramBoolean);
    if (arrayOfIndexUse.length == 0) {
      ...
      case 8: 
      case 11: 
        Index localIndex = createIndexForColumns(paramSession, paramOrderedIntHashSet.toArray());
        if (localIndex != null) {
          arrayOfIndexUse = localIndex.asArray();
        }
        break;
      }
    }
  ...
  Index.IndexUse[] findIndexForColumns(Session paramSession, OrderedIntHashSet paramOrderedIntHashSet, int paramInt, boolean paramBoolean)
  {
    ...
    while (i < j)
    {
      ...
      if (k != 0)
      {
        if (k == paramOrderedIntHashSet.size()) {
          return localIndex.asArray();
        }
        if ((k == localIndex.getColumnCount()) && (localIndex.isUnique())) {
          return localIndex.asArray();
        }
        if ((localObject.length == 0) && (k == localIndex.getColumnCount()))
        {
          localObject = localIndex.asArray();
        }
        else
        {
          ...
          arrayOfIndexUse[(arrayOfIndexUse.length - 1)] = new Index.IndexUse(localIndex, k);
          localObject = arrayOfIndexUse;
        }
      }
      i++;
    }
    return (Index.IndexUse[])localObject;
  }
  
  public Index getFullIndex(Session paramSession)
  {
    if (this.fullIndex == null)
    {
      this.fullIndex = getFullIndexForColumns(this.defaultColumnMap);
      if (this.fullIndex == null) {
        this.fullIndex = createIndexForColumns(paramSession, this.defaultColumnMap);
      }
    }
  ...
  public final long[] getIndexRootsArray()
  {
    ...
    long[] arrayOfLong = new long[this.indexList.length];
    for (int i = 0; i < this.indexList.length; i++)
    {
      CachedObject localCachedObject = localPersistentStore.getAccessor(this.indexList[i]);
      arrayOfLong[i] = (localCachedObject == null ? -1L : localCachedObject.getPos());
    }
    return arrayOfLong;
  }
  
  public void setIndexRoots(long[] paramArrayOfLong1, long[] paramArrayOfLong2, long paramLong)
  {
    if (!this.isCached) {
    ...
    PersistentStore localPersistentStore = this.database.persistentStoreCollection.getStore(this);
    for (int i = 0; i < this.indexList.length; i++) {
      localPersistentStore.setAccessor(this.indexList[i], paramArrayOfLong1[i]);
    }
    for (i = 0; i < this.indexList.length; i++) {
      localPersistentStore.setElementCount(this.indexList[i], paramLong, paramArrayOfLong2[i]);
    }
  }
  ...
  public void setIndexRoots(long[] paramArrayOfLong)
  {
    ...
    PersistentStore localPersistentStore = this.database.persistentStoreCollection.getStore(this);
    for (int i = 0; i < this.indexList.length; i++) {
      localPersistentStore.setAccessor(this.indexList[i], paramArrayOfLong[i]);
    }
  }
  ...
  void setIndexRoots(Session paramSession, String paramString)
  {
    ...
    int i = getIndexCount();
    ParserDQL localParserDQL = new ParserDQL(paramSession, new Scanner(paramSession, paramString), null);
    ...
    try
    {
      ...
      l1 = localParserDQL.readBigint();
    }
    catch (Exception localException) {}
    setIndexRoots(arrayOfLong1, arrayOfLong2, l1);
  }
  
  void generateAndCheckData(Session paramSession, Object[] paramArrayOfObject)
  {
  ...
  {
    generateAndCheckData(paramSession, paramArrayOfObject);
    if (this.isView) {
      return null;
    }
  ...
  void insertIntoTable(Session paramSession, Result paramResult)
  {
    ...
    {
      Object[] arrayOfObject1 = localRowSetNavigator.getNext();
  ...
  public int insertSys(Session paramSession, PersistentStore paramPersistentStore, Result paramResult)
  {
    ...
    for (int i = 0; localRowSetNavigator.hasNext(); i++) {
      insertSys(paramSession, paramPersistentStore, localRowSetNavigator.getNext());
    }
    return i;
  }
    ...
    if (this.hasGeneratedValues) {
      for (int i = 0; i < this.colGenerated.length; i++) {
        if (this.colGenerated[i] != 0)
        {
          ...
          localRangeIteratorBase.setCurrent(paramArrayOfObject);
  ...
  public void setUpdatedColumns(Session paramSession, Object[] paramArrayOfObject)
  {
    if (this.hasUpdatedValues) {
      for (int i = 0; i < this.colUpdated.length; i++) {
        if (this.colUpdated[i] != 0)
        {
          Expression localExpression = getColumn(i).getUpdateExpression();
          paramArrayOfObject[i] = localExpression.getValue(paramSession, this.colTypes[i]);
        }
      }
    }
  ...
  public void systemUpdateIdentityValue(Object[] paramArrayOfObject)
  {
    if (this.identityColumn != -1)
    {
      Number localNumber = (Number)paramArrayOfObject[this.identityColumn];
  ...
  public Row getDeleteRowFromLog(Session paramSession, Object[] paramArrayOfObject)
  {
    ...
    if (hasPrimaryKey())
    {
      localObject1 = getPrimaryIndex();
      localObject2 = ((Index)localObject1).getDefaultColumnMap();
      RowIterator localRowIterator = ((Index)localObject1).findFirstRow(paramSession, localPersistentStore, paramArrayOfObject, (int[])localObject2);
      localRow = localRowIterator.getNextRow();
      localRowIterator.release();
    }
    else if (this.bestIndex == null)
    {
      localObject1 = rowIterator(paramSession);
      for (;;)
      {
        localRow = ((RowIterator)localObject1).getNextRow();
        if (localRow != null) {
      ...
      ((RowIterator)localObject1).release();
    }
    else
    {
      localObject1 = this.bestIndex.findFirstRow(paramSession, localPersistentStore, paramArrayOfObject);
      for (;;)
      {
        localRow = ((RowIterator)localObject1).getNextRow();
        if (localRow == null) {
        ...
        localObject2 = localRow.getData();
        if (this.bestIndex.compareRowNonUnique(paramSession, (Object[])localObject2, paramArrayOfObject, this.bestIndex.getColumns()) != 0) {
          localRow = null;
        } else {
          if (compareRows(paramSession, (Object[])localObject2, paramArrayOfObject, this.defaultColumnMap, this.colTypes) == 0) {
            break;
          }
        }
      }
  ...
  public RowIterator rowIteratorClustered(Session paramSession)
  {
    ...
    return localIndex.firstRow(paramSession, localPersistentStore, 0, null);
  }
...
/* Location:              E:\java\java学习\hutubill\lib\all.jar!\org\hsqldb\Table.class
