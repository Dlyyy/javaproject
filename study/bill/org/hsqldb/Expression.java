  ...
  implements Cloneable
{
  ...
  public void setCorrelatedReferences(RangeGroup paramRangeGroup)
  {
    int i;
    if (this.rangeGroups == null)
    {
      for (i = 0; i < this.nodes.length; i++) {
        if (this.nodes[i] != null) {
          this.nodes[i].setCorrelatedReferences(paramRangeGroup);
        }
      }
    }
    else if (ArrayUtil.find(this.rangeGroups, paramRangeGroup) > -1)
    {
      for (i = this.rangeGroups.length - 1; (i >= 0) && (this.rangeGroups[i] != paramRangeGroup); i--) {
  ...
  public void resolveTypes(Session paramSession, Expression paramExpression)
  {
    ...
    case 96: 
      ...
      this.table.prepareTable(paramSession);
  ...
  void insertValuesIntoSubqueryTable(Session paramSession, PersistentStore paramPersistentStore)
  {
    for (int i = 0; i < this.nodes.length; i++)
    {
      Object[] arrayOfObject1 = this.nodes[i].getRowValue(paramSession);
      Object[] arrayOfObject2 = paramPersistentStore.getTable().getEmptyRowData();
      for (int j = 0; j < this.nodeDataTypes.length; j++) {
        arrayOfObject2[j] = this.nodeDataTypes[j].convertToType(paramSession, arrayOfObject1[j], this.nodes[i].nodes[j].dataType);
      }
  ...
  static void convertToType(Session paramSession, Object[] paramArrayOfObject, Type[] paramArrayOfType1, Type[] paramArrayOfType2)
  {
    for (int i = 0; i < paramArrayOfObject.length; i++) {
      if (!paramArrayOfType1[i].canConvertFrom(paramArrayOfType2[i])) {
  ...
  {
    ParserDQL.CompileContext localCompileContext = new ParserDQL.CompileContext(paramSession);
    localCompileContext.setNextRangeVarIndex(0);
  ...
  static HsqlList resolveColumnSet(Session paramSession, RangeVariable[] paramArrayOfRangeVariable, int paramInt, RangeGroup[] paramArrayOfRangeGroup, HsqlList paramHsqlList1, HsqlList paramHsqlList2)
  {
    ...
    RangeGroup.RangeGroupSimple localRangeGroupSimple = new RangeGroup.RangeGroupSimple(paramArrayOfRangeVariable, false);
  ...
  static
  {
    ...
    subqueryAggregateExpressionSet.add(94);
...
/* Location:              E:\java\java学习\hutubill\lib\all.jar!\org\hsqldb\Expression.class
