...
public final class SortAndSlice
{
  ...
  void setSortIndex(QuerySpecification paramQuerySpecification)
  {
    ...
    for (int j = 0; j < this.columnCount; j++)
    {
      ...
      if (((Expression)localObject).getRangeVariable() != paramQuerySpecification.rangeVariables[0]) {
        return;
      }
  ...
  public boolean prepareSpecial(Session paramSession, QuerySpecification paramQuerySpecification)
  {
    ...
    if (localExpression.getRangeVariable() != paramQuerySpecification.rangeVariables[0]) {
      return false;
    }
    ...
    if (paramQuerySpecification.rangeVariables[0].hasAnyTerminalCondition()) {
      return false;
    }
    Object localObject;
    if (paramQuerySpecification.rangeVariables[0].hasSingleIndexCondition())
    {
      ...
      if (localObject[0] != localExpression.getColumnIndex()) {
        return false;
      }
      if (i == 74) {
    ...
    else
    {
      ...
      Index localIndex2 = ((Table)localObject).getIndexForColumn(paramSession, localExpression.getColumnIndex());
      if (localIndex2 == null) {
        return false;
      }
  ...
  int[] getLimits(Session paramSession, QueryExpression paramQueryExpression, int paramInt)
  {
    ...
    if ((paramQueryExpression instanceof QuerySpecification))
    {
      ...
      if (localQuerySpecification.isSimpleDistinct) {
        n = 1;
      }
    }
    if (m != 0)
  ...
  public Index getNewIndex(Session paramSession, TableBase paramTableBase)
  {
    if (hasOrder())
    {
      Index localIndex = paramTableBase.createAndAddIndexStructure(paramSession, null, this.sortOrder, this.sortDescending, this.sortNullsLast, false, false, false);
      if (this.collations != null) {
        for (int i = 0; i < this.columnCount; i++) {
...
/* Location:              E:\java\java学习\hutubill\lib\all.jar!\org\hsqldb\SortAndSlice.class
