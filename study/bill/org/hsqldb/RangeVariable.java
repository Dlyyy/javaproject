...
public class RangeVariable
{
  ...
  public RangeVariable(HashMappedList paramHashMappedList, HsqlNameManager.SimpleName paramSimpleName, boolean paramBoolean, int paramInt)
  {
    ...
    {
    case 2: 
      this.usedColumns = new boolean[paramHashMappedList.size()];
    case 3: 
    case 4: 
      break;
    default: 
  ...
  public void addNamedJoinColumnExpression(String paramString, Expression paramExpression, int paramInt)
  {
    ...
    this.namedJoinColumnCheck[paramInt] = true;
  }
  ...
  public boolean hasAnyTerminalCondition()
  {
    for (int i = 0; i < this.joinConditions.length; i++) {
      if (this.joinConditions[0].terminalCondition != null) {
        return true;
      }
    }
    for (i = 0; i < this.whereConditions.length; i++) {
      if (this.whereConditions[0].terminalCondition != null) {
        return true;
      }
    }
    return false;
  ...
  {
    if ((this.namedJoinColumnExpressions != null) && (this.namedJoinColumnExpressions.containsKey(paramString3)) && (paramString2 != null)) {
      return -1;
    }
  ...
  public void resetConditions()
  {
    ...
    localIndex = this.whereConditions[0].rangeIndex;
    this.whereConditions = new RangeVariableConditions[] { new RangeVariableConditions(this, false) };
    this.whereConditions[0].rangeIndex = localIndex;
  }
  ...
  public void resolveRangeTable(Session paramSession, RangeGroup paramRangeGroup, RangeGroup[] paramArrayOfRangeGroup)
  {
    ...
    if (localQueryExpression != null)
    {
      ...
      this.rangeTable.prepareTable(paramSession);
      setRangeTableVariables();
    }
  }
  ...
  void moveConditionsToInner(Session paramSession, RangeVariable[] paramArrayOfRangeVariable)
  {
    ...
    for (j = localHsqlArrayList.size() - 1; j >= 0; j--)
    {
      Expression localExpression2 = (Expression)localHsqlArrayList.get(j);
      if ((localExpression2 == null) || (localExpression2.isTrue()) || (localExpression2.hasReference(paramArrayOfRangeVariable, i))) {
    ...
    for (int k = 0; k < localHsqlArrayList.size(); k++)
    {
      ...
      localOrderedHashSet1 = localExpression3.collectAllSubqueries(localOrderedHashSet1);
    ...
    for (int m = 0; m < localHsqlArrayList.size(); m++)
    {
      Expression localExpression4 = (Expression)localHsqlArrayList.get(m);
      localExpression4 = localExpression4.duplicate();
      localExpression4 = localExpression4.replaceColumnReferences(this, arrayOfExpression);
      OrderedHashSet localOrderedHashSet2 = localExpression4.collectRangeVariables(null);
      if (localOrderedHashSet2 != null) {
        for (int n = 0; n < localOrderedHashSet2.size(); n++)
        {
          RangeVariable localRangeVariable = (RangeVariable)localOrderedHashSet2.get(n);
          if ((this != localRangeVariable) && (localRangeVariable.rangeType == 1))
          {
            localQueryExpression.setCorrelated();
            break;
          }
        }
      }
      localExpression1 = ExpressionLogical.andExpressions(localExpression1, localExpression4);
    }
  ...
  public String describe(Session paramSession, int paramInt)
  {
    ...
    localStringBuffer1.append(this.rangeTable.getRowStore(paramSession).elementCount()).append("\n");
  ...
  public RangeIteratorMain getIterator(Session paramSession)
  {
    ...
    return (RangeIteratorMain)localObject;
  }
  ...
  public static class RangeVariableConditions
  {
      ...
      case 43: 
        if (this.opType == 48)
        {
          if (arrayOfInt[(this.indexedColumnCount - 1)] == i)
          {
            ...
            if ((paramExpression.getType() == 42) && (this.indexedColumnCount == 1)) {
    ...
    {
      if (((this.opType == 40) || (this.opType == 47)) && (this.indexedColumnCount < this.rangeIndex.getColumnCount()) && (this.rangeIndex.getColumns()[this.indexedColumnCount] == paramExpression.getLeftNode().getColumnIndex()))
      {
    ...
    void addIndexCondition(Expression[] paramArrayOfExpression, Index paramIndex, int paramInt)
    {
      ...
      case 43: 
        this.indexCond = paramArrayOfExpression;
        if (paramArrayOfExpression[0].getType() == 42) {
          this.indexEndCond[0] = (this.indexEndCondition = paramArrayOfExpression[0].nodes[2]);
        }
        this.opTypeEnd = 74;
        this.opTypesEnd[0] = 74;
        break;
      ...
      case 45: 
        ...
        this.indexEndCond[0] = (this.indexEndCondition = paramArrayOfExpression[0]);
      ...
      case 47: 
        ...
        for (int j = 0; j < paramInt; j++)
        {
          ...
          this.opTypes[j] = localExpression.opType;
          this.opTypesEnd[j] = localExpression.opType;
        }
        this.opTypeEnd = this.opType;
        break;
      case 46: 
    ...
    private void reverseIndexCondition()
    {
      ...
      if ((this.opType != 40) && (this.opType != 47))
      {
        ...
        Expression[] arrayOfExpression = this.indexCond;
        this.indexCond = this.indexEndCond;
        this.indexEndCond = arrayOfExpression;
        int[] arrayOfInt = this.opTypes;
        this.opTypes = this.opTypesEnd;
        this.opTypesEnd = arrayOfInt;
        for (int i = 0; i < this.indexedColumnCount; i++)
        {
          Expression localExpression = this.indexEndCond[i];
          this.indexEndCondition = ExpressionLogical.andExpressions(this.indexEndCondition, localExpression);
        }
        if ((this.indexedColumnCount > 1) && (this.opTypes[(this.indexedColumnCount - 1)] == 74))
        {
          this.indexedColumnCount -= 1;
    ...
    private void replaceColumnReferences(RangeVariable paramRangeVariable, Expression[] paramArrayOfExpression)
    {
      int i;
  ...
  public static class RangeIteratorRight
    extends RangeVariable.RangeIteratorMain
  {
    ...
    extends RangeVariable.RangeIteratorBase
  {
    ...
    protected void initialiseIterator()
    {
      ...
      if (this.conditions[this.condIndex].indexCond == null)
      {
        if (this.conditions[this.condIndex].reversed) {
          this.it = this.conditions[this.condIndex].rangeIndex.lastRow(this.session, this.store, this.rangeVar.indexDistinctCount, null);
        } else {
          this.it = this.conditions[this.condIndex].rangeIndex.firstRow(this.session, this.store, this.rangeVar.indexDistinctCount, null);
        }
      }
      else
      {
    ...
    private void getFirstRow()
    {
      ...
      int i = this.conditions[this.condIndex].opType;
      for (int j = 0; j < this.conditions[this.condIndex].indexedColumnCount; j++)
      {
        int k = 0;
        int m = this.conditions[this.condIndex].opTypes[j];
        if ((m == 47) || (m == 48) || (m == 74))
        {
          this.currentJoinData[j] = null;
        }
        else
        {
          Type localType1 = this.conditions[this.condIndex].indexCond[j].getRightNode().getDataType();
          Object localObject = this.conditions[this.condIndex].indexCond[j].getRightNode().getValue(this.session);
          Type localType2 = this.conditions[this.condIndex].indexCond[j].getLeftNode().getDataType();
          ...
          {
            k = localType2.compareToTypeRange(localObject);
            if ((k == 0) && (localType2.typeComparisonGroup != localType1.typeComparisonGroup)) {
              localObject = localType2.convertToType(this.session, localObject, localType1);
            }
          }
          if (j == 0)
          {
            int n = this.conditions[this.condIndex].indexCond[0].getType();
            if (k < 0)
            {
              switch (n)
              {
          ...
          this.currentJoinData[j] = localObject;
        }
      }
      this.it = this.conditions[this.condIndex].rangeIndex.findFirstRow(this.session, this.store, this.currentJoinData, this.conditions[this.condIndex].indexedColumnCount, this.rangeVar.indexDistinctCount, i, this.conditions[this.condIndex].reversed, null);
    }
    ...
    implements RangeIterator
  {
    ...
    {
      return this.currentRow == null ? null : this.currentRow.getTable();
    }
...
/* Location:              E:\java\java学习\hutubill\lib\all.jar!\org\hsqldb\RangeVariable.class
