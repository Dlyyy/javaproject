  ...
  implements HsqlSocketRequestHandler, Notified
{
  ...
  public final void notify(int paramInt)
  {
    printWithThread("notify( database shutdown," + paramInt + ") entered");
    releaseDatabase(paramInt);
    int i = 1;
  ...
  final boolean openDatabases()
  {
    ...
    for (int i = 0; i < this.dbAlias.length; i++) {
      if (this.dbAlias[i] != null)
      {
        ...
        localStopWatch.stop();
        String str = "Database [index=" + i + ", id=" + j + ", db=" + this.dbType[i] + this.dbPath[i] + ", alias=" + this.dbAlias[i] + "] opened successfully";
  ...
  private IntKeyHashMap getDBNameArray()
  {
    ...
    while (localEnumeration.hasMoreElements())
    {
      ...
      if (str1.startsWith("server.dbname."))
      {
        ...
        {
          printWithThread("malformed database enumerator: " + str1);
        }
        continue;
  ...
  private class ServerThread
    extends Thread
  {
    ServerThread(String paramString)
    {
      super();
      setName(paramString + '@' + Integer.toString(Server.this.hashCode(), 16));
    }
...
/* Location:              E:\java\java学习\hutubill\lib\all.jar!\org\hsqldb\server\Server.class
