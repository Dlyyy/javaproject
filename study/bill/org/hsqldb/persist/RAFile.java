  ...
  implements RandomAccessInterface
{
    ...
    throws FileNotFoundException, IOException
  {
    if (paramInt == 3) {
      try
      {
        ...
        Object localObject2 = localConstructor.newInstance(new Object[] { paramString, Boolean.valueOf(paramBoolean), str2 });
        ...
        if ((localObject2 instanceof Storage)) {
          return new RAStorageWrapper((Storage)localObject2);
        }
    ...
    if (paramInt == 5)
    {
      localObject1 = new RAFile(paramDatabase.logger, paramString, paramBoolean, false, true);
      return (RandomAccessInterface)localObject1;
    }
    if (paramInt == 0) {
      return new RAFile(paramDatabase.logger, paramString, paramBoolean, true, false);
    }
  ...
  RAFile(EventLogInterface paramEventLogInterface, String paramString, boolean paramBoolean1, boolean paramBoolean2, boolean paramBoolean3)
    throws FileNotFoundException, IOException
  {
  ...
  private void readIntoBuffer()
    throws IOException
  {
    long l1 = this.seekPosition & 0xFFFFFFFFFFFFF000;
    ...
    {
      resetPointer();
      this.logger.logWarningEvent("Read Error " + l1 + " " + l2, localIOException);
    ...
    throws IOException
  {
    ...
    {
      resetPointer();
      this.logger.logWarningEvent("read failed", localIOException);
      throw localIOException;
    }
  }
    ...
    throws IOException
  {
    ...
    {
      resetPointer();
      this.logger.logWarningEvent("failed to read a byte array", localIOException);
      throw localIOException;
    }
  }
    ...
    throws IOException
  {
    ...
    {
      resetPointer();
      this.logger.logWarningEvent("failed to write a byte array", localIOException);
      throw localIOException;
    }
  }
  ...
  public void synch()
  {
    ...
    catch (Throwable localThrowable1)
    {
      ...
      {
        this.logger.logSevereEvent("RA file sync error ", localThrowable2);
        throw Error.error(localThrowable1, 452, null);
      }
    }
  }
  
  private int writeToBuffer(byte[] paramArrayOfByte, int paramInt1, int paramInt2)
    throws IOException
  {
    int i = ArrayUtil.copyBytes(this.seekPosition - paramInt1, paramArrayOfByte, paramInt1, paramInt2, this.bufferOffset, this.buffer, this.buffer.length);
    return i;
  }
  ...
  private long getExtendLength(long paramLong)
  {
    ...
    paramLong = ArrayUtil.getBinaryNormalisedCeiling(paramLong, 12 + i);
    return paramLong;
  }
...
/* Location:              E:\java\java学习\hutubill\lib\all.jar!\org\hsqldb\persist\RAFile.class
