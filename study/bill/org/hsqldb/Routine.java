  ...
  implements SchemaObject, RangeGroup, Cloneable
{
  ...
  {
    ParserRoutine localParserRoutine = new ParserRoutine(paramSession, new Scanner(paramSession, this.statement.getSQL()));
    paramSession.sessionContext.pushRoutineTables();
    try
    {
    ...
    {
      paramSession.sessionContext.popRoutineTables();
    }
  }
  ...
  public void resolve(Session paramSession)
  {
    ...
    for (int i = 0; i < this.parameterTypes.length; i++)
    {
      ...
      if (i < 4) {
        this.typeGroups = BitMap.setByte(this.typeGroups, (byte)localColumnSchema.dataType.typeComparisonGroup, i * 8);
      }
    }
    if (this.isAggregate)
    {
  ...
  public void setAsAlteredRoutine(Routine paramRoutine)
  {
    ...
    this.cursorCount = paramRoutine.cursorCount;
    ...
    while (j + i < paramArrayOfObject2.length)
    {
      ...
      if (localObject2 != null) {
        if ((localObject2 instanceof JDBCResultSet))
        {
          ...
          {
            ((Result)localObject1).addChainedResult(localResult);
          }
        }
        else
        {
  ...
  public Result invokeJavaMethodDirect(Object[] paramArrayOfObject)
  {
    ...
    {
      localResult = Result.newErrorResult(Error.error(localThrowable, 6000, getName().name));
    }
    return localResult;
  }
  ...
  Result invokeJavaMethod(Session paramSession, Object[] paramArrayOfObject)
  {
    ...
    {
      localResult = Result.newErrorResult(Error.error(localInvocationTargetException, 6000, getName().name));
    }
    catch (IllegalAccessException localIllegalAccessException)
    {
      localResult = Result.newErrorResult(Error.error(localIllegalAccessException, 6000, getName().name));
    }
    catch (Throwable localThrowable)
    {
      localResult = Result.newErrorResult(Error.error(localThrowable, 6000, getName().name));
  ...
  public Result invoke(Session paramSession, Object[] paramArrayOfObject1, Object[] paramArrayOfObject2, boolean paramBoolean)
  {
    ...
    if (isPSM())
    {
      try
      {
        ...
        paramSession.sessionContext.routineCursors = Result.emptyArray;
        if (this.cursorCount > 0) {
          paramSession.sessionContext.routineCursors = new Result[this.cursorCount];
        }
    ...
    for (i = 0; i < arrayOfMethod.length; i++)
    {
      ...
      if (m == paramRoutine.parameterTypes.length)
      {
        ...
        else
        {
          ...
          if ((localType1 == null) || (!paramRoutine.returnType.canBeAssignedFrom(localType1))) {
            continue;
          }
          if ((localType1.isLobType()) || ((!localType1.isBinaryType()) && (!localType1.isCharacterType())))
          {
            i2 = paramRoutine.returnType.typeCode;
            if (i2 == 2) {
              i2 = 3;
            }
            if (localType1.typeCode != i2) {
              continue;
            }
          }
        }
        for (int i1 = 0; i1 < paramRoutine.parameterTypes.length; i1++)
        {
          ...
          if ((localClass.isArray()) && (!byte[].class.equals(localClass)))
          {
          ...
          {
            int i4 = paramRoutine.parameterTypes[i1].typeCode;
            if (i4 == 2) {
              i4 = 3;
            }
            i3 = i4 == localType2.typeCode ? 1 : 0;
          }
    ...
    for (i = 0; i < arrayOfMethod.length; i++)
    {
      ...
      if ((localMethod.getName().equals(str2)) && (Modifier.isStatic(m)) && (Modifier.isPublic(m)))
      {
        ...
        for (int n = j; n < arrayOfClass.length; n++)
        {
          ...
          {
            if (!byte[].class.equals(localClass2))
            {
  ...
  public static Routine newRoutine(Session paramSession, Method paramMethod)
  {
    ...
    for (int j = i; j < arrayOfClass.length; j++)
    {
      ...
      HsqlNameManager.HsqlName localHsqlName = paramSession.database.nameManager.newHsqlName("C" + (j - i + 1), false, 23);
      ColumnSchema localColumnSchema = new ColumnSchema(localHsqlName, localType2, !arrayOfClass[j].isPrimitive(), false, null);
...
/* Location:              E:\java\java学习\hutubill\lib\all.jar!\org\hsqldb\Routine.class
